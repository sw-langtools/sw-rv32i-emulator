...
use sw_rv32i_isa::{ImmOp, Instruction, IsaProfile, Reg, StoreWidth, disassemble};
use sw_rv32i_target::{Board, GenericUartMmio, MmioBus, load_board_file};
...
fn run_board(path: &Path) -> Result<(), String> {
    ...
    let program = uart_hello_program(uart_base)?;
...
fn uart_hello_program(uart_base: u32) -> Result<Vec<u8>, String> {
    let mut insns = Vec::new();
    insns.extend(load_const(Reg::X1, uart_base));
    for byte in MESSAGE {
        insns.extend([
            Instruction::OpImm {
                op: ImmOp::Addi,
            ...
            Instruction::Store {
                ...
                rs1: Reg::X1,
                rs2: Reg::X2,
                offset: GenericUartMmio::TXDATA_OFFSET as i32,
            },
        ]);
    }
    insns.push(Instruction::Ebreak);
